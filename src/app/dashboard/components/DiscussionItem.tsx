import { DiscussTypeRedux, fetchDiscussionsThunk } from "@/redux/slices/discussSlice";
...
import deleteDiscussService from "@/app/discussion/services/deleteDiscussService";
...
function DiscussionItem(data: DiscussTypeRedux) {
  ...
  async function handleDeleteClick() {
    await deleteDiscussService(data.docId);
  ...
  useEffect(() => {
    ...
    const liked = data.likes
      ? data.likes.filter((item) => item === user?.user_id).length > 0
      : false;
    setIsLiked(liked);
  }, [data.likes, user]);
    ...
    <div className="cursor-pointer p-5 w-full rounded-2xl bg-[#262D34] xl:min-w-[620px] 2xl:min-w-[720px]">
      <div className="flex w-full gap-3 xl:gap-4 2xl:gap-5">
        <Link href={`/discussion/${data.docId}`}>
          <img
            className="min-w-[80px] max-w-[80px] h-20 md:min-w-[156px] md:max-w-[156px] md:h-[156px] object-cover block rounded-2xl"
            src={data.image}
            alt=""
          />
        </Link>
          ...
          <Link href={`/discussion/${data.docId}`}>
            <div className="flex gap-2 mt-[10px]">
              {data.hashtag.map((hashtag, index) => (
                <div
                  key={index}
                  className="px-[10px] py-1 rounded-[20px] text-[10px] text-[#C5D0E6] bg-[#2C353D]"
                >
                  {hashtag}
                </div>
              ))}
            ...
            <div className="hidden md:flex mt-7 justify-between">
              <div className="flex gap-[10px]">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#C5D0E6]">
                    {getSinceCreated()} ago
                  </p>
                </div>
              </div>
              <div className="flex gap-3 h-fit my-auto">
                ...
                <p className="text-sm text-[#C5D0E6]">
      ...
      <button onClick={handleDeleteClick} className="transition-all px-4 py-2 bg-red-500 rounded-lg mt-3 w-full hover:bg-red-400">
        Delete Content
      </button>
    </div>
  );
